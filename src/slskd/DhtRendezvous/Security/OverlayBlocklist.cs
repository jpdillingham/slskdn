...
public sealed class OverlayBlocklist : IDisposable
{
    ...
    private readonly ConcurrentDictionary<IPAddress, ViolationRecord> _violationTracker = new();
    ...
    /// </summary>
    public static readonly TimeSpan ViolationWindow = TimeSpan.FromHours(1);
    
    /// <summary>
    /// Number of violations within window before auto-ban.
    /// </summary>
    public const int ViolationsBeforeAutoBan = 5;
    
    /// <summary>
    /// Number of auto-bans before permanent ban.
    /// </summary>
    public const int AutoBansBeforePermanent = 3;
    ...
    public BlocklistStats GetStats()
    {
        ...
        return new BlocklistStats
        {
            ...
            TrackedViolators = _violationTracker.Count,
            TotalViolationsRecorded = _violationTracker.Values.Sum(v => v.TotalViolations),
        };
    }
    
    /// <summary>
    /// Record a security violation from an IP address.
    /// May trigger automatic escalating bans.
    /// </summary>
    /// <param name="ip">The IP address.</param>
    /// <param name="violationType">Type of violation.</param>
    /// <param name="details">Additional details.</param>
    /// <returns>Action taken (none, warned, banned, permanent).</returns>
    public ViolationAction RecordViolation(IPAddress ip, ViolationType violationType, string? details = null)
    {
        var now = DateTimeOffset.UtcNow;
        var record = _violationTracker.GetOrAdd(ip, _ => new ViolationRecord());
        ...
        lock (record)
        {
            ...
            _logger.LogDebug(
                "Violation recorded for {Ip}: {Type} ({Count} in window, {Total} total)",
                ip, violationType, recentCount, record.TotalViolations);
            ...
            if (recentCount >= ViolationsBeforeAutoBan)
            {
                ...
                {
                    // Permanent ban
                    BlockIp(ip, $"Auto-permanent: {record.TotalViolations} violations, {record.AutoBanCount} auto-bans", permanent: true);
                    _logger.LogWarning(
                        "PERMANENT BAN for {Ip}: {Total} total violations, {AutoBans} auto-bans",
                        ip, record.TotalViolations, record.AutoBanCount);
                    return ViolationAction.PermanentBan;
                }
                else
                {
                    // Escalating temporary ban
                    var banDuration = TimeSpan.FromHours(Math.Pow(2, record.AutoBanCount)); // 2h, 4h, 8h, etc.
                    BlockIp(ip, $"Auto-ban #{record.AutoBanCount}: {recentCount} violations in {ViolationWindow.TotalHours}h", banDuration);
                    _logger.LogWarning(
                        "Auto-ban #{AutoBan} for {Ip}: {Count} violations, banned for {Duration}",
                        record.AutoBanCount, ip, recentCount, banDuration);
    ...
    /// </summary>
    public ViolationRecord? GetViolationRecord(IPAddress ip)
    {
        return _violationTracker.TryGetValue(ip, out var record) ? record : null;
    }
    
    /// <summary>
    /// Get all IPs with recorded violations.
    /// </summary>
    public IReadOnlyDictionary<IPAddress, ViolationRecord> GetViolationRecords()
    {
        return _violationTracker.ToDictionary(kvp => kvp.Key, kvp => kvp.Value);
    }
...
public sealed class BlocklistStats
{
    ...
    public int TrackedViolators { get; init; }
    public long TotalViolationsRecorded { get; init; }
